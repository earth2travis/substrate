...
async function checkTweetHighEngagement(sb, conditions) {
  ...
  return {
    ...
    proposal: {
      ...
      proposed_steps: [
        { kind: "analyze", payload: { topic: "tweet-performance" } },
      ],
    },
  };
}
...
async function applyPairwiseDrifts(drifts, policy, conversationId) {
  for (const { agent_a, agent_b, drift, reason } of drifts) {
    ...
    await sb
      ...
      .update({
        affinity: clamp(currentAffinity + clamped, 0.1, 0.95),
        drift_log: [
          ...recentLog.slice(-19),
          { drift: clamped, reason, conversationId, at: new Date() },
        ],
      })
      .eq("agent_a", a)
      .eq("agent_b", b);
...
1. Heartbeat identifies "this agent is due for an initiative"
2. → writes to ops_initiative_queue
3. → VPS worker consumes the queue
4. → Haiku model generates proposals (cheap + fast)
5. → POST /api/ops/proposals (goes through full proposal-service gates)
...
| Policy                       | Suggested Value                                     | Purpose                                |
| ---------------------------- | --------------------------------------------------- | -------------------------------------- |
| ⚙️ auto_approve              | `{ "enabled": true }`                               | Enable auto-approval                   |
| ⚙️ x_daily_quota             | `{ "limit": 5 }`                                    | Daily tweet limit (start conservative) |
| ⚙️ roundtable_policy         | `{ "enabled": true, "max_daily_conversations": 5 }` | Conversation cap (start conservative)  |
| ⚙️ memory_influence_policy   | `{ "enabled": true, "probability": 0.3 }`           | Memory influence probability           |
| ⚙️ relationship_drift_policy | `{ "enabled": true, "max_drift": 0.03 }`            | Max relationship drift                 |
| ⚙️ initiative_policy         | `{ "enabled": false }`                              | Keep off until the system is stable    |
...
You can see all 6 agents operating in real time at [voxyz.space](https://voxyz.space).
