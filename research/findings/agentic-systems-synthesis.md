...
# Agentic Systems: Three-Layer Architecture for Production Agents
...
Symphony's insight: do not build complex agent-to-agent routing. Instead, use an external work queue (issue tracker, task system) and dispatch isolated agent sessions. The orchestration is in the infrastructure, not in the agents.

This maps directly to how we already work. Our issue tracker, workspace isolation, and bounded concurrency via sub-agents: we are already doing Symphony's architecture informally. Loom should formalize it.
...
Do not let agents decide the workflow. Let them execute within it.

### 3. Skills are the missing middleware

MCP gives you tools. Workflows give you scheduling. But between "I can query a database" and "process this customer request" there is a knowledge gap: the procedural expertise of knowing which tools to use in what order with what error handling.

Skills fill this gap. They are lightweight (markdown files), portable (cross-platform), and composable. The Anthropic format is becoming standard.
...
MCP is moving to stateless (June 2026 spec). The "hosted MCP server" business model is on borrowed time as complexity simplifies to REST-like patterns. Design for stateless from day one.

## Recommendations for Synthweave/Loom

**Adopt:**
- **MCP** for all tool integrations. It is the standard.
- **Anthropic Skill format** for reusable procedural knowledge.
...
**Build:**
- **Permission/budget enforcement layer**: The gap in the market.
- **Skill library**: Every recurring workflow as a skill.
- **Stateless MCP servers**: Prepare for the June 2026 spec.

**Skip:**
- **CrewAI's role abstraction**: Define agents by capabilities, not characters.
- **AutoGen's conversation-as-computing**: Too unpredictable for production.
- **LangGraph as a dependency**: Implement the state graph pattern directly.
- **Framework-specific tool abstractions**: Go straight to MCP.

**Watch:**
- **MCP Apps**: Full applications as MCP servers.
- **MCP Sampling with tool support**: Server-initiated LLM calls that can use tools.
- **Agent AI Foundation**: Now governs MCP, Skills, agents.md.
...
This is not a framework. It is an architecture. The components are simple, the interfaces are standard, and each piece can evolve independently.

The key differentiator: we are not building another CrewAI or LangGraph. We are building the operational layer that makes agents actually work in production, with the permission model and budget enforcement that nobody else has built yet.
