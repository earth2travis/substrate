...
|---|---|---|---|
| `research/raw/` | Agents, humans (manual placement) | `_ingest.py`, `_lint.py`, agent synthesis | Append-only, immutable after write |
| `research/findings/` | Agent synthesis (cron) | `_lint.py`, humans, agents | Written by agent; not regenerated by `_ingest.py` |
| `research/queries/` | Humans, agents | `_query.py` | Append/edit manually |
| `insights/concepts/` | Agent synthesis (cron), agents (manual) | Humans, agents | Stable once promoted; not overwritten by ingest |
| `insights/entities/` | Agent synthesis (cron), agents (manual), entity map | Humans, agents | Stable once promoted; entity map is manually curated |
...
| `retros/` | `_retro.sh`, agents (manual) | Humans, agents | Append-only, immutable once written |
...
| `scripts/` | Agent/system | Humans, agents | Append/edit manually; version controlled |
...
### 3.3 Stage 2: Process (Mechanical)

`_ingest.py` generates a machine-readable digest of all raw sources:

1. Scan `research/raw/*.md`
2. Parse existing frontmatter leniently (extracts what exists; does not validate)
3. Compute sha256 of file body for drift detection
4. Compare to previous run's hashes stored in `.ingest-state/`
5. Classify each file: new, changed, unchanged, html-excluded, no-ingest-excluded
6. Detect orphaned findings (raw source deleted since last run)
7. Write JSON digest

The script does NOT synthesize findings, auto-tag, or promote to insights. Those require semantic understanding and belong in the agent layer (Stage 3).

The pipeline runs daily on cron (06:00 UTC). See `specs/ingest-spec.md` for the full specification.

### 3.4 Stage 3: Synthesize (Agent-Driven)

The synthesis agent reads the ingest digest and performs the semantic work that `_ingest.py` cannot:

1. **Identify what changed** — new_files + changed_files from the digest
2. **Summarize each source** — write synthesized findings to `research/findings/`
3. **Cross-reference** — discover entities and concepts, link via `[[...]]` wikilinks
4. **Promote** — when a topic appears across 2+ sources or is central, create/update insight pages in `insights/`
5. **Update navigation** — add entries to `INDEX.md`, append to `log.md`
6. **Commit** — branch, commit, push, open PR

Early exit: if the digest shows nothing new, the agent exits clean without creating a branch or PR.

**Why this split works:**
Scripts are fast and deterministic but cannot understand content. Agents understand content but are stateful and expensive. The boundary is the JSON digest: mechanical work produces it, semantic work consumes it.
...
Some promotions are straightforward (high cross-reference density). Others require manual synthesis by an agent because the insight needs judgment to construct. Auto-promoted insights use the body from the most representative finding, enriched with cross-references.

Promoted insights are stable. They are not overwritten by subsequent ingest runs.

**Note on entity resolution:** Entity aliasing (e.g., "Ξ2T" = "earth2travis") is handled during agent synthesis, not by `_ingest.py`. When an entity map exists at `insights/entities/entity-map.json`, the synthesis agent loads it and resolves aliases while writing findings and insights.
...
| Job | Schedule | Runner | Output |
|---|---|---|---|
| Synthesis | Daily 06:00 UTC | Agent (cron) | `research/findings/` + `insights/` updates, PR opened |

**Target state (not yet implemented):**

| Job | Schedule | Runner | Output |
|---|---|---|---|
| Lint | Daily 02:00 UTC | Script | stdout (exit code signals pass/fail) |
| Ingest digest | Daily 02:30 UTC | Script | JSON digest to `/tmp/` |
| Security scan | Daily 03:00 UTC | Script | stdout (exit code signals pass/fail) |
| Eval | Sun 04:00 UTC | Script | `evals/YYYY-MM-DD-eval.md` |
| Retro generation | Sun 05:00 UTC | Script | `retros/week-YYYY-WNN.md` |

Only the synthesis job is active. The others are aspirational and require `_scan.py`, `_eval.py`, and `_retro.sh` to be built. See the `scan-spec.md`, `eval-spec.md`, and `retro-spec.md` for specifications.
...
- **Wikilinks:** Use `[[...]]` syntax for all internal references.
